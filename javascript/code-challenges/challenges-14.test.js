...
const screenForNames = (arr) => {
    ...
    let newArr=arr.filter (str => {
      if (str.startsWith('Mr. ') || str.startsWith('Mrs. ') || str.startsWith('Ms. ') || str.startsWith('Dr. ')){
         return str
      }
    });
    newArr.pop();
...
const toTitleCase = (arr) => {
  let newArr=arr.map(word => {
    ...
    let newWord=word.replace(word.charAt(0) , capitalLetter);
    return newWord;
  });
  return newArr;
};
...
let biggerThanLuke = (arr) => {
  let charName;
    let newArr=arr.filter(character => {
        if (Number(character.mass) > 77) {
         charName=character.name
        }
        return charName
    })
   let lastArr = newArr.map(char => {
       return char.name
   })
   let str= lastArr.join('');
      let str2 = str.replace(/Leia Organa/,' - ');
      return str2;
}
...
const isSecure = (url) => {
  let security ='https://'
  let bool = url.startsWith(security) 
  return bool
