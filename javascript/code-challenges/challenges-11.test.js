...
function transformToLis(obj){
  ...
  for (let [key, value] of Object.entries(obj)) {
    let whatever= `<li>${key}: ${value}</li>`
    newArr.push(whatever)
  }
  return newArr;
}
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  let bigArr=[];
  bigArr = input.map(integer=>{
integer = integer.filter(element => {
  if (element % 5 === 0 && typeof(element) === 'number'){
      return true
    } 
    
    })
integer=integer.map(whatever => {
  whatever = Math.pow(2, whatever);
  return whatever
})
return integer;
  });
...
let findMaleAndFemale = (data) => {
  ...
  let finalResult=newStr.replace(/,/g, ' and ');
