...
const toLastNames = people => {
  let newArr = [];
  people.map(name => {
    newArr.push(name.firstName + ' ' + name.lastName);
  });
  return newArr;
};
...
const addPurchases = (arr) => {
  let newArr = arr.reduce((acc, price) => {
    acc = acc + price.purchasePrice;
    return acc;
  }, 0);
  return newArr;
};
...
const returnNames = (arr) => {
  let newArr = [];
  arr.reduce((acc, character) => {
    acc = acc + character.name;
...
const reversedString = (str) => {
  let arr = str.split('');
  let newArr = arr.reduce((acc, letter) => {
    acc = letter + acc;
    return acc;
  }, '');
...
const countNumberOfChildren = (arr) => {
  let numOfChildren = arr.reduce((counter, family) => {
    if (family.hasOwnProperty('children')) {
      counter = counter + family.children.length;
    }
    return counter;
  }, 0);
...
const countPrimeNumbers = (arr) => {
  let primeNumbers = arr.reduce((counter, num) => {
    if (isPrime(num)) {
      counter = counter + 1;
    }
    return counter;
...
const extractStat = (statName, arr) => {
  let objects = arr.reduce((acc, item) => {
...
const extractChildren = (arr) => {
  ...
  // let aIncludedCharacters;
  let aIncludedCharacters = newArr.reduce((acc, character) => {
